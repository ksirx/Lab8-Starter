...
async function getRecipes() {
  ...
  const curr = localStorage.getItem('recipes');
  // checks if empty
  if(curr != null) {
    // returns the array of all recipes if found.
  ...
  // a promise which will be returned
  const promise = new Promise(async function(resolve, reject) {
    ...
    for(let i = 0; i < RECIPE_URLS.length; i++) {
      ...
      try {
        ...
        // if at the end, the carries out the above
